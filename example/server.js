...
app.post("/events", (req, res) => {
  ...
  const normalizedAvailability = String(availability || "free").trim().toLowerCase();
  const trimmedWhere = String(where || "").trim();

  if (!title || !when || (normalizedAvailability !== "busy" && !trimmedWhere)) {
    return res.status(400).json({
      error: normalizedAvailability === "busy"
        ? "Title and when are required."
        : "Title, when, and where are required."
    });
  }
...
app.put("/events/:id", (req, res) => {
  ...
  const trimmedWhere = String(where || "").trim();
  if (!title || !when || (event.availability !== "busy" && !trimmedWhere)) {
    return res.status(400).json({
      error: event.availability === "busy"
        ? "Title and when are required."
        : "Title, when, and where are required."
    });
  }
